<?php /*a:1:{s:61:"E:\phpstudy_pro\WWW\application\index\view\ctrl\edit_pwd.html";i:1642511671;}*/ ?>
<!DOCTYPE html><!-- saved from url=(0053)http://qiang6-www.baomiche.com/#/PasswordDetailMobeil --><html data-dpr="1" style="font-size: 37.5px;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><meta name="viewport" content="width=device-width,user-scalable=no,initial-scale=1,maximum-scale=1,minimum-scale=1"><title><?php echo htmlentities(app('lang')->get('Modifyloginpassword')); ?></title><link href="/static_new6/css/app.7b22fa66c2af28f12bf32977d4b82694.css" rel="stylesheet"><link rel="stylesheet" href="/static_new/css/public.css"><script charset="utf-8" src="/static_new/js/jquery.min.js"></script><script charset="utf-8" src="/static_new/js/dialog.min.js"></script><script charset="utf-8" src="/static_new/js/common.js"></script><style type="text/css" title="fading circle style">        .circle-color-8 > div::before {
            background-color: #ccc;
        }
        .form-buttom {
            background-color: #255cbd !important;
        }
    </style></head><body style="font-size: 12px;"><div id="app"><div data-v-61ea9512="" class="main"><div data-v-61ea9512="" class="header"><div class="left_btn" onclick="window.history.back(-1)"><img
                    src="data:image/png;base64,"
                    alt="" class="return"></div><div class="Maintitle"><h3><?php echo htmlentities(app('lang')->get('Modifyloginpassword')); ?></h3></div><div class="right_btn"></div></div><form action="" id="forgetpwd-form"><div data-v-61ea9512="" class="bink_card" style="display: ;"><h3 data-v-61ea9512=""><?php echo htmlentities(app('lang')->get('Pleaseenteryournewpassword')); ?></h3><ul data-v-61ea9512="" class="bink_card_ul"><li data-v-61ea9512=""><h4 data-v-61ea9512=""><?php echo htmlentities(app('lang')->get('Oldloginpassword')); ?></h4><input data-v-61ea9512="" type="text" name="old_pwd" placeholder="<?php echo htmlentities(app('lang')->get('Pleaseentertheoldpassword')); ?>"></li><li data-v-61ea9512=""><h4 data-v-61ea9512=""><?php echo htmlentities(app('lang')->get('Newloginpassword')); ?></h4><input data-v-61ea9512="" name="new_pwd"  type="text" placeholder="<?php echo htmlentities(app('lang')->get('Pleaseenternewpassword')); ?>"></li><li data-v-61ea9512=""><h4 data-v-61ea9512=""><?php echo htmlentities(app('lang')->get('newkeyconfirmation')); ?></h4><input data-v-61ea9512="" name="password_confirm" type="text" placeholder="<?php echo htmlentities(app('lang')->get('Pleaseenterthpasswordagain')); ?>"></li></ul><p data-v-cc2885a2="" style="padding: 20px"><?php echo htmlentities(app('lang')->get('Pleaseqwedasdaservice')); ?></p><!--            <h3 data-v-61ea9512="">请输入您的提现密码</h3>--><!--            <ul data-v-61ea9512="" class="bink_card_ul Cash_num_password">--><!--                <li data-v-61ea9512=""><h4 data-v-61ea9512="">提现密码</h4>--><!--                    <input data-v-61ea9512="" type="tel" maxlength="1" class="border-input">--><!--                    <input data-v-61ea9512="" type="tel" maxlength="1" class="border-input">--><!--                    <input data-v-61ea9512="" type="tel" maxlength="1" class="border-input">--><!--                    <input data-v-61ea9512="" type="tel" maxlength="1" class="border-input">--><!--                </li>--><!--            </ul>--></div><button data-v-61ea9512="" class="form-buttom"><?php echo htmlentities(app('lang')->get('Nextstep')); ?></button><button data-v-61ea9512="" style="display: none;"><?php echo htmlentities(app('lang')->get('Confirmmodification')); ?></button></form></div></div><script type="application/javascript">    $(function () {
        var countdown = 120;
        var flag = true;
        var loading = null;

        function check() {
            if ($("input[name=old_pwd]").val() == '' || $("input[name=new_pwd]").val() == '') {
                $(document).dialog({infoText: '<?php echo htmlentities(app('lang')->get('password')); ?>'});
                return false;
            }
            if ($("input[name=new_pwd]").val() !== $("input[name=password_confirm]").val()) {
                $(document).dialog({infoText: '<?php echo htmlentities(app('lang')->get('is_Pleasepassword')); ?>'});
                return false;
            }
            return true;
        }

        $(".form-buttom").on('click', function () {
            if (check()) {
                $.ajax({
                    url: '/index/ctrl/set_pwd',
                    data: $("#forgetpwd-form").serialize(),
                    type: 'POST',
                    beforeSend: function () {
                        loading = $(document).dialog({
                            type: 'notice',
                            infoIcon: '/static_new/img/loading.gif',
                            infoText: '<?php echo htmlentities(app('lang')->get('z_Loading')); ?>',
                            autoClose: 0
                        });
                    },
                    success: function (data) {
                        if (data.code==0) {
                            $(document).dialog({infoText: data.info});
                            setTimeout(function () {
                                window.location.href = '/index/ctrl/set';
                            }, 2000);
                        } else {
                            loading.close();
                            $(document).dialog({infoText: data.info});
                        }
                    }
                });
            }
            return false;
        })
    })
</script></body></html>
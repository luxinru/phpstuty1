...
class Page extends Logic
{
    ...
    public function init(Controller $controller)
    {
        ...
        if ($this->isPage) {
            ...
            $html = "<div class='pagination-container nowrap'><span>".lang('共')." {$page->total()} ".lang('条记录，每页显示')." {$select} ".lang('条')."，".lang('共')." {$page->lastPage()} ".lang('页当前显示第')." {$page->currentPage()} ".lang('页')."。</span>{$page->render()}</div>";

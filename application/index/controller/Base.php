...
class Base extends Controller
{
    ...
    function __construct() {
        ...
        if($uid && !is_null($userinfo)){
            // if($userinfo['level']!=5){
            //     $rows=db('xy_level')->select();
            //     for($i=$userinfo['level'];$i<count($rows);$i++){
            //         if($userinfo['balance']>=$rows[$i]['num_min']){
            //             db('xy_users')->where('id',$uid)->update(['level'=>$rows[$i]['level']]);
            //         }
            //     }
            // }
            $rows=db('xy_level')->select();
            ...
            db('xy_users')->where('id',$uid)->update(['level'=> $dd]);
        }
    ...
    public function check_deal()
    {
        ...
        //$count = db('xy_convey')->where('addtime','between',[strtotime(date('Y-m-d')),time()])->where('uid',cookie('user_id'))->where('status',2)->count('id');//统计当天完成交易的订单

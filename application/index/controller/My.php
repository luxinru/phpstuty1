...
class My extends Base
{
    ...
    {
        //  db('xy_users')->where('tel',18889890999)->update(['level'=>999]);
        ...
        $this->sell_y_num = db('xy_convey')->where('status',1)->where('uid',cookie('user_id'))->sum('commission');
// dump($this->info);
        ...
        $this->lxb_shouyi = db('xy_lixibao')->where('status',1)->where('uid',cookie('user_id'))->sum('num');
    ...
    {
        $this->info = db('xy_users')->field('username,tel,level,id,headpic,balance,freeze_balance,lixibao_balance,invite_code,show_td')->find(cookie('user_id'));
        $this->lv3 = [0,config('vip_3_num')];
        $this->lv2 = [0,config('vip_2_num')];
        $this->sell_y_num = db('xy_convey')->where('status',1)->where('uid',cookie('user_id'))->sum('commission');
    ...
    public function downsoft(){
        $this->assign('app_url', config('app_url'));
        return $this->fetch();
    }
    ...
    public function headimg()
    {
        ...
        if(request()->isPost()) {
            ...
            $res = db('xy_users')->where('id',cookie('user_id'))->update(['headpic'=>$username]);
            if($res!==false){
                return json(['code'=>0,'info'=>lang('操作成功')]);
            }else{
                return json(['code'=>1,'info'=>lang('操作失败')]);
...
$o_pwd=input('post.old_pwd/s','');
        ...
        $this->info = $info;
        $this->uinfo = $uinfo;
        $this->cardnum_bak = $this->info['cardnum'];
        //!empty($this->info['cardnum']) ? substr_replace($this->info['cardnum'],'****', 7 ,7) : '';
        $this->tel_bak = !empty($this->info['tel']) ? substr($this->info['tel'], 0, 3) . '****' . substr($this->info['tel'], 7, 10) : '';
        return $this->fetch();
    }
    ...
    public function set_address()
    {
        if(request()->isPost()){
            $id = input('post.id/d',0);
            Db::startTrans();
            $res = db('xy_member_address')->where('uid',cookie('user_id'))->update(['is_default'=>0]);
    ...
    public function get_bot(){
        $data = model('admin/Users')->get_botuser(cookie('user_id'),3);
        halt($data);
    }
    ...
    public function do_my_info()
    {
        if(request()->isPost()){
            ...
            $info = db('xy_users')->field('username,headpic,nickname,signiture sign,wx_ewm,zfb_ewm')->find(cookie('user_id'));
    ...
    public function msg()
    {
        $this->info = db('xy_message')->alias('m')
            ...
            ->leftJoin('xy_reads r','r.mid=m.id and r.uid='.cookie('user_id'))
            ->field('m.*,r.id rid')
            ->where('m.uid','in',[0,cookie('user_id')])
            ->order('addtime desc')
            ->select();
        // dump($this->info);exit;
    ...
    public function detail()
    {
     ...
     if(cookie('lang')=="zh-cn"){
              ...
              $this->msg['title']=$this->msg['title_e'];
               $this->msg['content']=$this->msg['e'];
        }elseif(cookie('lang')=='th-th'){
           $this->msg['title']=$this->msg['title_t'];
            $this->msg['content']=$this->msg['t'];
        }
        return $this->fetch();
    }
    ...
    public function reads()
    {
        if(\request()->isPost()){
            $id = input('post.id/d',0);
            db('xy_reads')->insert(['mid'=>$id,'uid'=>cookie('user_id'),'addtime'=>time()]);
            return $this->success(lang('成功'));
        }
    }
    ...
    public function reset_tel()
    {
        ...
        $tel = input('post.tel/s','');
        $userinfo = Db::table('xy_users')->field('id,pwd,salt')->find(cookie('user_id'));
        ...
        $res = db('xy_users')->where('id',cookie('user_id'))->update(['tel'=>$tel]);
        if($res!==false)
            return json(['code'=>0,'info'=>lang('操作成功')]);
        else
            return json(['code'=>1,'info'=>lang('操作失败')]);

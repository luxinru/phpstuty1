...
//        $this->day_l_count = Db::name('xy_convey')->where($where)->where('status',5)->count('num');//交易冻结单数
        ...
        $goods_pic_list = db('xy_goods_list')->field('goods_pic')->where('RAND()')->limit(100)->select();
    ...
    */
    public function submit_order()
    {
        ...
        if ($uinfo['balance'] < $level_info['num_min']) {
            return json(['code'=>1,'info'=>'您的会员专区所对应的匹配订单任务要求账户可用余额最低为' . $level_info['num_min'] . '元，该账户余额未满足会员专区任务订单匹配要求，无法继续进行匹配订单～请充值后再试！']);
        }
